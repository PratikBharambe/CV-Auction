...
const AddVehicle = () => {
    ...
    const handleFileChange = (e) => {
        ...
        if (name === 'images') {
            setFormData({
                ...formData,
                images: Array.from(files) // Convert file list to an array for multiple images
            });
        } else {
            setFormData({
                ...formData,
                [name]: files[0] // Single file (PDF)
            });
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        data.append('manufacturereName', formData.manufacturereName);
        ...
        data.append('yearofregistration', formData.yearofregistration);
        ...
        if (formData.pdf) {
            ...
            formDataPdf.append('upload_preset', 'your_upload_preset_for_pdf'); // For PDF storage

            try {
                const pdfResponse = await axios.post('your_pdf_upload_endpoint', formDataPdf, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                data.append('pdf', pdfResponse.data.secure_url); // Push the PDF URL from S3 or Cloudinary
            } catch (err) {
