...
const VehiclesPage = () => {
  ...
  const [vehicleDetails, setVehicleDetails] = useState(null); // State to hold specific vehicle details
  const [highestBids, setHighestBids] = useState({}); // State to track the highest bid for each vehicle
  const navigate = useNavigate();

  // Function to fetch vehicle data
  useEffect(() => {
    axios.get('/api/vehicles')  // Make the GET request to your API
      .then((response) => {
        setVehicles(response.data);  // Set the vehicle data from API to state
        const initialBids = {};
        response.data.forEach(vehicle => {
          initialBids[vehicle.id] = vehicle.highestBid || 0; // Initialize with existing highest bids from DB
        });
        setHighestBids(initialBids); // Set initial highest bids for all vehicles
      })
      .catch((error) => {
  ...
  const fetchVehicleDetails = (vehicleId) => {
    axios.get(`/api/vehicles/${vehicleId}`) // Make a GET request for the vehicle details
      .then((response) => {
        setVehicleDetails(response.data); // Set the vehicle details to the state
  ...
  const handleWatchlistClick = () => {
    navigate('/watchlist');  // Redirects to Watchlist page
  };
  ...
  const handlePlaceBid = (vehicleId) => {
    ...
    if (bidAmount > highestBids[vehicleId]) {
      ...
      axios.post(`/api/vehicles/${vehicleId}/bid`, { bidAmount })
        .then(response => {
