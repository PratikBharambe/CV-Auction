...
const CurrentAuctionPage = () => {
  ...
  const [price, setPrice] = useState(0); // Store price input
  ...
  const getName = async (auctionid) => {
    try {
      ...
      if (error.response) {
        console.error("Error:", error.response.data); // Backend error message
      } else {
        console.error("Error:", error.message); // Network or unexpected errors
      }
    }
  };
